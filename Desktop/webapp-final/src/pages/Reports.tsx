...
export function Reports() {
  ...
  // Group by category
  const incomeByCat = incomeTxs.reduce((acc, t) => {
    acc[t.Category] = (acc[t.Category] || 0) + Number(t.Currency === 'UZS' ? t.Amount_UZS : t.Amount_USD);
    return acc;
  }, {} as Record<string, number>);

  const expenseByCat = expenseTxs.reduce((acc, t) => {
    acc[t.Category] = (acc[t.Category] || 0) + Number(t.Currency === 'UZS' ? t.Amount_UZS : t.Amount_USD);
    return acc;
  }, {} as Record<string, number>);
  ...
  return (
    <div className="space-y-6 pb-4">
      <h2 className="text-2xl font-extrabold tracking-tight text-gray-900 mb-6 flex justify-between items-end">
        <span>Отчеты <span className="text-[13px] font-normal text-gray-500 ml-1">/ Hisobotlar</span></span>
      </h2>
      ...
      <div className="flex overflow-x-auto pb-2 gap-2 hide-scrollbar">
        {[
          ...
          <button
            ...
            className={`whitespace-nowrap px-4 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${
              period === p.id ? 'bg-gray-900 text-white' : 'bg-white text-gray-900 border border-gray-200 hover:bg-gray-50'
            }`}
          >
            {p.label}
          </button>
